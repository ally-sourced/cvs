...
foreach ($sub in $subs) {
    ...
    Write-Progress -Activity "AKS Admission/Runtime Assessment" -Status ("Subscription {0}/{1}: {2}" -f $subIndex, $subTotal, $subName) -PercentComplete ([int](($subIndex/$subTotal)*100))
    Write-Host ""
    ...
    $clusterIndex = 0
    $clusterTotal = (Get-Count $clusters)

    foreach ($c in $clusters) {
        $clusterIndex++

        $rg = ""; $name = ""; $location = ""; $k8sVersion = ""
        ...
        Write-Progress -Activity ("Subscription: {0}" -f $subName) -Status ("Cluster {0}/{1}: {2}" -f $clusterIndex, $clusterTotal, $name) -PercentComplete ([int](($clusterIndex/$clusterTotal)*100))
        ...
        if ([string]::IsNullOrWhiteSpace($clusterId)) { try { $clusterId = $c.id } catch { $clusterId = "" } }
        ...
        # --- 3) In-cluster admission evidence
        ...
        # --- 5) Map policy assignment IDs to names (best effort)
        ...
        $evidenceObject = [PSCustomObject]@{
            metadata = [PSCustomObject]@{
                ...
                tool = "Assess-AKS-AdmissionRuntime-AllSubs-OneEvidencePerCluster-PS51.ps1"
            }
